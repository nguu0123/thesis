...
class Qoa_Client(object):
    ...
    def init_metric(self, name, configuration: dict):
        ...
        if configuration["class"] == "Gauge":
            ...
            return Histogram(
                name, configuration["description"], configuration["default"]
            )
    ...
    def asyn_report(
        self, metrics: list = None, report: dict = None, connectors: list = None
    ):
        client_inf = self.config.copy()
        client_inf["timestamp"] = str(datetime.fromtimestamp(time.time()))
        client_inf["runtime"] = time.time() - self.start_time
        ...
        self.lock.acquire()
        if connectors == None:
            self.connector[self.default_connector].send_data(
